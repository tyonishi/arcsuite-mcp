...
- RSA credential handling must use the server challenge and public key with
  RSA PKCS#1 v1.5 padding (`RSA/ECB/PKCS1Padding`) over the UTF-8 bytes of
  `challenge + password`. Passwords and the gateway token are read from secret
  files when configured, with environment variables only as a controlled
  local-development fallback; neither is ever logged or returned.
