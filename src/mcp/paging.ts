...
export class PagingSnapshotStore {
  ...
  create(input: {
    ...
    this.pruneExpired();
    if (!Number.isSafeInteger(input.pageSize) || input.pageSize < 1 || input.pageSize > this.maxIdsPerSnapshot) {
    ...
    if (boundedIds.length <= input.pageSize) {
      return { ids: first, nextCursor: null, snapshotLimited, context: input.context };
    }
    ...
    return {
      ids: first,
      nextCursor: this.createCursor(snapshot, input.pageSize),
      snapshotLimited,
  ...
  next(cursor: string, expected: { clientProfileId: string; scopeId: string; kind: PagingKind }): PagingPage {
    ...
    if (payload.client_profile_id !== expected.clientProfileId || payload.scope_id !== expected.scopeId || payload.kind !== expected.kind) {
      throw new Error("PAGING_CURSOR_SCOPE_MISMATCH");
    }
    ...
    if (snapshot.clientProfileId !== expected.clientProfileId || snapshot.scopeId !== expected.scopeId || snapshot.kind !== expected.kind) {
      throw new Error("PAGING_CURSOR_SCOPE_MISMATCH");
    }
    if (payload.page_size !== snapshot.pageSize || payload.offset < 0 || payload.offset >= snapshot.ids.length) {
      throw new Error("INVALID_PAGING_CURSOR");
    }
  ...
  private createCursor(snapshot: PagingSnapshot, offset: number): string {
    const payload: PagingCursorPayload = {
      ...
      client_profile_id: snapshot.clientProfileId,
  ...
  private parseCursor(cursor: string): PagingCursorPayload {
    ...
    try { parsed = JSON.parse(Buffer.from(body, "base64url").toString("utf8")) as PagingCursorPayload; }
    catch { throw new Error("INVALID_PAGING_CURSOR"); }
    if (
      parsed.v !== 1 || !parsed.snapshot_id || !parsed.client_profile_id || !parsed.scope_id ||
      (parsed.kind !== "search" && parsed.kind !== "folder") ||
      !Number.isSafeInteger(parsed.offset) || !Number.isSafeInteger(parsed.page_size) || !Number.isSafeInteger(parsed.expires_at)
    ) throw new Error("INVALID_PAGING_CURSOR");
  ...
  private sign(body: string): string {
    return createHmac("sha256", this.secret).update(`paging-v1.${body}`).digest("base64url");
  }

  private pruneExpired(): void {
    const now = Date.now();
    for (const snapshot of this.snapshots.values()) {
      if (snapshot.expiresAt <= now) this.remove(snapshot.id);
    }
  }
  ...
  private makeRoom(clientProfileId: string, incomingIds: number): void {
    ...
    while ([...this.snapshots.values()].filter((s) => s.clientProfileId === clientProfileId).length >= this.maxSnapshotsPerClient) {
      const victim = byAge().find((s) => s.clientProfileId === clientProfileId);
      if (!victim) break;
      this.remove(victim.id);
    }
    while (this.snapshots.size >= this.maxSnapshots || this.totalIds + incomingIds > this.maxTotalIds) {
      const victim = byAge()[0];
      if (!victim) break;
      this.remove(victim.id);
    }
    if (this.snapshots.size >= this.maxSnapshots || this.totalIds + incomingIds > this.maxTotalIds) {
      throw new Error("PAGING_CACHE_LIMIT");
    }
  }
